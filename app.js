(() => {
  ...
  function jsonp(baseUrl, params) {
    return new Promise((resolve, reject) => {
      ...
      const timer = window.setTimeout(() => { cleanup(); reject(new Error("Google Apps Script did not respond within 45 seconds.")); }, 45000);
      const cleanup = () => { window.clearTimeout(timer); try { delete window[callback]; } catch (_) {} script.remove(); };
      window[callback] = (data) => { cleanup(); data && data.ok ? resolve(data) : reject(new Error((data && data.error) || "Unknown Apps Script error.")); };
      const url = new URL(baseUrl);
      Object.entries({ ...params, callback }).forEach(([k, v]) => url.searchParams.set(k, String(v)));
      script.src = url.toString();
      script.onerror = () => { cleanup(); reject(new Error("Unable to connect to the Apps Script endpoint.")); };
  ...
  async function loadStatus(nextEndpoint) {
    if (!nextEndpoint || nextEndpoint.includes(GAS_PLACEHOLDER)) {
      ...
      setText("connection-message", "Demo stock loaded for review"); renderSource(); renderMetrics(); return;
    }
    if (!/^https:\/\/script\.google\.com\/.+\/exec(?:\?|$)/i.test(nextEndpoint)) {
      state.isConnected = false; renderSource(); setText("connection-message", "Invalid Apps Script /exec URL"); showMessage("Please paste the deployed Google Apps Script Web App URL ending in /exec.", "error"); return;
    }
    ...
    try {
      ...
      setText("connection-message", "Could not reach Apps Script");
  ...
  function bindEvents() {
    ...
    $("connect-btn").addEventListener("click", async () => { const next=$("endpoint-input").value.trim(); state.endpoint=next; if(next)localStorage.setItem("loneeddy-gas-url",next); else localStorage.removeItem("loneeddy-gas-url"); await loadStatus(next); });
